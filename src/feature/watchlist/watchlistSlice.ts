...
let watchArray = JSON.parse(retStringArray) !== null ? JSON.parse(retStringArray) : [];
...
export const watchlistSlice = createSlice({
  ...
  reducers: {
    addToWatchlist: (state: any, action: any) => {
      ...
      let existsItemIndex = state.watchlistItems?.findIndex((item: any)=>item.id ===action.payload?.id);
    ...
    removeFromWatchlist: (state: any, action: any) => {
      let filteredItems = state.watchlistItems?.filter((item: any)=>item?.id !== action.payload?.id );
